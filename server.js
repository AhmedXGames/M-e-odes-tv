...
setInterval(() => {
  http.get(`http://great-birch-jjxu96vi4k.glitch.me/`);
}, 280000);
...
function play(guild, song) {
  ...
  if(serverQueue.repeating) {
	...
	dispatcher.setVolumeLogarithmic(serverQueue.volume / 100);
      ...
      if(isNaN(args[0])) return msg.channel.send(`:notes: The current volume is ${queue.volume}`);

      if(parseInt(args[0]) > 150) return msg.channel.send('You can\'t set the volume more than 100.')
...
client.on('message', message => {
    ...
    let argresult = message.content.split(` `).slice(1).join(' ');//تغير حالات بوت
...
client.on('message', message => {
  ...
  message.author.send( `**:notes:  قائمة الاوامر:  
